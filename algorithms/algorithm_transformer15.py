...
class IndexModule(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 1)
        resulting_indices = (x + self.offsets)
        x = self.linterp(resulting_indices.flatten())
        x = x.reshape(self.batch_size, main_indices, -1)
        return x, resulting_indices
...
class ANN(nn.Module):
    def __init__(self, target_size, mode):
        ...
        self.index_embedding = nn.Sequential(nn.Linear(d_model, d_model), nn.GELU())
...
class Algorithm_transformer15(Algorithm):
